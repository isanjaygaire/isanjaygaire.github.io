...
class posts extends WP_Widget {
	...
	function widget( $args, $instance ) {
        ...
        $readmorelinktext = esc_html__('Read more!','geographic');
        $compilepopular = '';
		...
		$compilepopular .= '
			<li '.((has_post_thumbnail()) ? '' : 'class="no_img"').'><div class="recent_posts_container">';
                if (has_post_thumbnail()) {
                    $compilepopular .= '<div class="recent_posts_img"><img src="'.aq_resize($featured_image_latest[0], "100", "100", true, true, true).'" alt="'.get_the_title().'"></div>';
                }
...
$compilepopular .= '
					<div class="recent_posts_content">
						<h6 class="post_title"><a class="read_more" href="'.get_permalink().'">'.get_the_title().'</a></h6>
						<div class="featured_items_meta">
							<span>'. get_the_time("M d, Y") .'</span>
							<span class="middot">&middot;</span>
							<span><a href="' . get_comments_link() . '">'. get_comments_number(get_the_ID()) .' '. esc_html__('comments', 'geographic') .'</a></span>
						</div>
					</div>
					<div class="clear"></div>
				</div>                
